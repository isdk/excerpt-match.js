...
describe('三档语义差异', () => {
  it('★ strict 不跨块，default 跨块', () => {
    ...
    const doc = '第一段甲\n\n第二段乙\n\n第三段丙';
    expect(locateExcerpt('甲第二', doc, { markdown: md, preset: 'strict' }).kind).toBe('none');
    expect(locateExcerpt('甲第二', doc, { markdown: md, preset: 'default' }).kind).not.toBe('none');
  });
  ...
  it('★ loose 忽略标点，default 不忽略', () => {
    ...
    expect(locateExcerpt(excerpt, doc, { preset: 'default' }).kind).toBe('none');
    expect(locateExcerpt(excerpt, doc, { preset: 'loose' }).kind).not.toBe('none');
  });
...
describe('预设落到真实定位行为', () => {
  it('strict 能命中精确摘录', () => {
    const r = locateExcerpt('被告的行为', DOC, { preset: 'strict' });
    expect(r.kind).toBe('exact');
    expect(DOC.slice(r.index, r.index + r.length)).toBe('被告的行为');
  });
  ...
  it('★ preset 与显式选项组合使用', () => {
    ...
    const doc = '第一段甲\n\n第二段乙\n\n第三段丙';
    expect(
      locateExcerpt('甲第二', doc, { markdown: md, preset: 'default', allowCrossBlock: false }).kind
    ).toBe('none');
  });

  it('preset 对 createTextIndex 同样生效', async () => {
    const { createTextIndex } = await import('./locator');
    const idx = createTextIndex(DOC, { preset: 'strict' });
