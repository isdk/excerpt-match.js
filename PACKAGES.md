...
1. **seed-and-extend**：摘录里挑"在页面中出现次数最少"的种子 → Bitap 定位 → 双向扩展。
   Bitap 对长 pattern 效果差，直接扔整段进去不稳。挑种子时避开**毒化种子**：
   摘录混入文档侧不存在的字符（md 标记 `**`、压坏的列表序号、OCR 噪声）时，
   含它们的窗口在原文出现 0 次 —— 「不存在」不是「最稀有」，Bitap 会空手而归。
   真定位不到时适配层会放宽阈值重试（proximity 惩罚是坏位置估计的产物，
   不是匹配质量），最终命中质量仍由 diff 分数把关。
